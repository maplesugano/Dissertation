...
In this project, the \textbf{emotion code} applies steering vectors to control the emotion level of the output. We first decompose emotions such as joy/anger/sadness/surprise, define pre-named primitive emotion vectors, and express diverse pre-verbal states as linear combinations. For instance, the vector sum of joy and surprise can yield a code close to ``excitement.'' Vector arithmetic within emotion space thus enables continuous, multidimensional specification of emotion. Unlike coarse labels, the code allows numerical tuning of subtle tones. Functioning as an axis orthogonal to conceptual content, the same emotion code derived from text can be applied to image generation or speech synthesis~-- enabling cross-modal transfer of emotional tone.

\section{Development Schedule}

For the final product, the goal is to ensure that the results of actually manipulating the emotion vectors detected from the input text, as revealed through user testing, manifest as outputs that intuitively make sense.

Research on steering vectors will fundamentally proceed using a GPT-2 model based on SIM-CoT operating with Python, aiming for release as a website with a React frontend and a .NET-based backend.
...
\toprule
\textbf{Tasks} & Jan & Feb & Mar & Apr & May \\
\midrule
Dataset Preparation     & = & = &   &   &   \\
Develop Emotion Codes   & = & = & = &   &   \\
Text Steering           &   & = & = & = &   \\
API \& Demo Release     &   &   &   & = & = \\
\bottomrule
\end{tabular}
\caption{Development Schedule from 16/1/2026 to 31/5/2026}
\end{table}

\begin{itemize}
  \item \textbf{Dataset Preparation}: Make a dataset that elicits a large model emotion.
  \item \textbf{Develop Emotion Codes}: establish Emotion Code v0.1.
  \item \textbf{Text Steering}: Improve on the text input → emotion vector editing → steered output process.
  \item \textbf{API \& Demo Release}: Web demo with nice visualisation, and final report writing.
\end{itemize}
